...
// Dependencies
const { userNameIsValid, phoneIsValid, passwordIsValid, tosAgreementIsValid, idIsValid, extendIsValid, tokenIsValid } = require('../utils/handlers');
...
handlers._users.get = function(data, cb) {
    ...
    if (checkedPhone) {
        ...
        const token = tokenIsValid(data.headers.token) ? data.headers.token : false;
        // verify that the given token from headers is valid for given phone number
